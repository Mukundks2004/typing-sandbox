  ...
  ReceiveKey(key: string): string {
    ...
    if (key === "\x1B[A") {
      if (this.stackPointer > 0) {
        ...
        return (
          "\x1b[1M" + this.prompt + this.activeRepl.history[this.stackPointer]
        );
      }
      return "";
    }
    ...
    if (key === "\x7F") {
      if (this.cursor > 0) {
        this.currentLine = removeNthCharacter(
          this.currentLine,
          this.cursor - 1
        );
        this.cursor--;
    ...
    if (")}]".includes(key)) {
      if (
        ...
        this.oppositeBrace[this.braceStack[this.braceStack.length - 1]] !== key
      ) {
        return key + "\n" + "Unexpected bracket\n" + this.prompt;
      }
      this.braceStack.pop();
